...
export default function SessionExitProvider({ children }: { children: React.ReactNode }) {
  ...
  const confirmExit = async () => {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : 'Could not exit the game.');
    } finally {
      setIsBusy(false);
    }
  };
  ...
  const resolveExit = async (action: 'discard' | 'save', mode?: 'new' | 'current') => {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : 'Could not process session data.');
    } finally {
      setIsBusy(false);
    }
  };
  ...
  const authenticateAnonymous = async (mode: 'login' | 'signup') => {
    ...
    try {
      ...
      if (!prepared?.handoffToken) throw new Error('Could not prepare session data transfer.');
      ...
      if (mode === 'login') {
        ...
        if (!result.session) {
          throw new Error('Confirm your email, then log in to save the session.');
        }
      }
  ...
  const entityName = state.pending?.role === 'master' ? 'world' : 'character';
        ...
        <div className="fixed inset-0 z-[1000] grid place-items-center bg-black/75 p-5 backdrop-blur-sm">
          <section className="w-full max-w-[460px] rounded-[26px] border border-white/12 bg-[#172033] p-[24px] shadow-2xl">
            <h2 className="font-montserrat-alt text-[26px] font-extrabold text-white">
              {confirmation.role === 'master' ? 'End this session?' : 'Exit the game?'}
            </h2>
            <p className="mt-[10px] font-montserrat text-[14px] leading-relaxed text-white/60">
              {confirmation.role === 'master'
                ? 'The room will close for everyone. Each participant will then choose what to do with their game data.'
                : 'You will leave the table and then choose what to do with your character.'}
            </p>
            ...
            <div className="mt-[22px] flex justify-end gap-[10px]">
              ...
              <button type="button" disabled={isBusy} onClick={() => void confirmExit()} className="rounded-[12px] bg-[#D6B25E] px-[18px] py-[11px] font-montserrat text-[13px] font-extrabold text-black disabled:opacity-50">
                {isBusy ? 'Processing…' : confirmation.role === 'master' ? 'End session' : 'Exit game'}
              </button>
            </div>
          </section>
        </div>
          ...
          <section className="relative w-full max-w-[590px] overflow-hidden rounded-[30px] border border-white/10 bg-[#172033] shadow-[0_32px_90px_rgba(0,0,0,.55)]">
            ...
            <div className="p-[22px] sm:p-[30px]">
              <div className="flex items-start gap-[15px]">
                ...
                <div className="min-w-0 pt-[1px]">
                  <p className="font-montserrat text-[10px] font-extrabold uppercase tracking-[.24em] text-white/45">Session ended</p>
                  <h2 className="mt-[5px] font-montserrat-alt text-[25px] font-extrabold leading-[1.15] text-white sm:text-[29px]">Keep your {entityName}?</h2>
                </div>
              </div>
              <div className="mt-[20px] flex items-center gap-[12px] rounded-[16px] border border-white/[.07] bg-[#0E1627]/75 px-[15px] py-[12px]">
                ...
                <div>
                  <p className="font-montserrat text-[10px] font-bold uppercase tracking-[.12em] text-white/35">Temporary data available until</p>
            ...
            {state.isAnonymous ? (
              <div className="mt-[18px] rounded-[20px] border border-white/10 bg-white/[.035] p-[16px] sm:p-[18px]">
                <p className="font-montserrat text-[13px] leading-relaxed text-white/65">
                  Sign in or create an account to keep this {entityName}. Otherwise, its temporary data will be removed.
                </p>
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email" className="mt-[14px] h-[46px] w-full rounded-[13px] border border-white/10 bg-[#0B1020] px-[14px] font-montserrat text-[14px] text-white outline-none transition focus:border-white/30" />
                <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" className="mt-[9px] h-[46px] w-full rounded-[13px] border border-white/10 bg-[#0B1020] px-[14px] font-montserrat text-[14px] text-white outline-none transition focus:border-white/30" />
                <div className="mt-[12px] grid grid-cols-2 gap-[9px]">
                  <button type="button" disabled={isBusy} onClick={() => void authenticateAnonymous('login')} className="rounded-[13px] border border-white/15 px-[14px] py-[12px] font-montserrat text-[13px] font-bold text-white transition hover:bg-white/[.05] disabled:opacity-40">Log in</button>
                  <button type="button" disabled={isBusy} onClick={() => void authenticateAnonymous('signup')} className="rounded-[13px] bg-white px-[14px] py-[12px] font-montserrat text-[13px] font-extrabold text-[#172033] transition hover:bg-white/90 disabled:opacity-40">Create account</button>
                </div>
              </div>
            ) : (
              <div className={`mt-[18px] grid gap-[10px] ${canSaveCurrent ? 'sm:grid-cols-2' : ''}`}>
                <button type="button" disabled={isBusy} onClick={() => void resolveExit('save', 'new')} className="group flex min-h-[68px] items-center justify-between rounded-[17px] bg-white px-[18px] text-left transition hover:bg-white/90 disabled:opacity-40">
                  <span><span className="block font-montserrat text-[14px] font-extrabold text-[#172033]">Save as new</span><span className="mt-[2px] block font-montserrat text-[10px] font-semibold text-[#172033]/55">Create a separate copy</span></span>
                ...
                {canSaveCurrent ? (
                  <button type="button" disabled={isBusy} onClick={() => void resolveExit('save', 'current')} className="group flex min-h-[68px] items-center justify-between rounded-[17px] border border-white/15 bg-white/[.045] px-[18px] text-left transition hover:bg-white/[.08] disabled:opacity-40">
                    <span><span className="block font-montserrat text-[14px] font-extrabold text-white">Update current</span><span className="mt-[2px] block font-montserrat text-[10px] font-semibold text-white/40">Replace the existing version</span></span>
            ...
            <div className="mt-[18px] border-t border-white/[.07] pt-[14px]">
              <button type="button" disabled={isBusy} onClick={() => void resolveExit('discard')} className="w-full rounded-[14px] px-[18px] py-[11px] font-montserrat text-[13px] font-bold text-white/45 transition hover:bg-white/[.04] hover:text-[#E88A8A] disabled:opacity-40">
                Don&apos;t save this {entityName}
              </button>
            </div>
            {isBusy ? <p className="mt-[12px] text-center text-[12px] text-white/45">Processing…</p> : null}
