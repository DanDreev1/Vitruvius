...
export default function StudioWorkspace<TTab extends StudioTab>({ role, tabs, renderContent }: StudioWorkspaceProps<TTab>) {
  const router = useRouter();
  const t = useTranslations('Studio');
  const s = useTranslations('StudioShell');
  ...
  const pluralLabel = role === 'master' ? t('worlds') : t('characters');
  const entityTitle = role === 'master' ? s('world') : s('character');
  const pluralTitle = pluralLabel.charAt(0).toUpperCase() + pluralLabel.slice(1);
  const tabTitles: Record<StudioTab, string> = {
    character: s('tabs.character'), skills: s('tabs.skills'), backpack: s('tabs.backpack'), library: s('tabs.library'),
    relationships: s('tabs.relationships'), notes: s('tabs.notes'), settings: s('tabs.settings'), scene: s('tabs.scene'), assets: s('tabs.assets'),
  };
  ...
  const confirmDelete = async () => {
    ...
    } catch (deleteError) { setError(deleteError instanceof Error ? deleteError.message : s('deleteError')); }
  ...
  return (
    <ScaledPageViewport headerBackdrop>
        ...
        <main className="flex h-[780px] items-center justify-center px-6 py-8">
          <section className="flex h-[660px] w-full max-w-[1120px] flex-col rounded-[28px] border border-white/[.08] bg-[#141D2E] p-6">
            <div className="flex items-center justify-between border-b border-white/[.07] pb-5">
              <div><Link href="/studio" className="group inline-flex items-center gap-2.5 font-montserrat text-[13px] font-extrabold text-white transition hover:opacity-70"><Image src="/Logo_Icon.png" alt="" width={25} height={25} className="h-[25px] w-[25px] object-contain transition-transform duration-300 group-hover:-translate-y-0.5" /><span>Vitruvius Studio</span></Link><h1 className="mt-3 font-montserrat-alt text-[34px] font-extrabold text-white">{role === 'master' ? s('yourWorlds') : s('yourCharacters')}</h1><p className="mt-1 font-montserrat text-[13px] text-white/55">{role === 'master' ? s('chooseWorld') : s('chooseCharacter')}</p></div>
              <span className="rounded-full border border-white/10 px-4 py-2 font-montserrat text-[12px] font-bold text-white/45">{role === 'master' ? s('countWorlds', {count: entities.length}) : s('countCharacters', {count: entities.length})}</span>
            </div>

            <div className="mt-5 min-h-0 flex-1 overflow-y-auto pr-2">
              {isLoading ? <div className="grid h-full place-items-center text-white/45">{role === 'master' ? s('loadingWorlds') : s('loadingCharacters')}</div> : entities.length ? (
                <div className="grid grid-cols-3 gap-4">
                  {entities.map((entity) => <article key={entity.id} className="group rounded-[21px] border border-white/[.08] bg-[#182135] p-4 transition hover:border-white/20"><button type="button" onClick={() => setSelected(entity)} className="w-full text-left"><div className="h-[130px] w-full rounded-[16px] bg-[#0D1525] bg-cover bg-center" style={entity.avatarUrl ? { backgroundImage: `url(${entity.avatarUrl})` } : undefined}>{!entity.avatarUrl ? <div className="flex h-full items-center justify-center font-montserrat-alt text-[42px] font-extrabold uppercase text-white/15">{entity.name.charAt(0)}</div> : null}</div><h2 className="mt-4 truncate font-montserrat-alt text-[20px] font-extrabold text-white">{entity.name}</h2><p className="mt-1 font-montserrat text-[11px] text-white/35">{role === 'master' ? s('openWorld') : s('openCharacter')} →</p></button><button type="button" onClick={() => setDeleteTarget(entity)} className="mt-4 w-full rounded-[11px] border border-red-300/10 py-2 font-montserrat text-[11px] font-bold text-red-300/45 transition hover:bg-red-400/[.06] hover:text-red-300">{role === 'master' ? s('deleteWorld') : s('deleteCharacter')}</button></article>)}
                </div>
              ) : <div className="grid h-full place-items-center text-center"><div><h2 className="font-montserrat-alt text-[28px] font-extrabold text-white">{role === 'master' ? s('noWorlds') : s('noCharacters')}</h2><p className="mt-2 text-[13px] text-white/45">{role === 'master' ? s('createWorld') : s('createCharacter')}</p></div></div>}
            </div>
            <button type="button" onClick={() => void createEntity()} disabled={isBusy} className="mt-5 flex h-[58px] shrink-0 items-center justify-center gap-3 rounded-[16px] bg-white font-montserrat text-[14px] font-extrabold text-[#172033] transition hover:bg-white/90 disabled:opacity-40"><span className="text-[25px] font-normal leading-none">+</span>{role === 'master' ? s('addWorld') : s('addCharacter')}</button>
      ...
      {deleteTarget ? <ViewportPortal><div className="fixed inset-0 z-[1000] grid place-items-center bg-[#070C17]/85 p-5 backdrop-blur-md"><section className="w-full max-w-[450px] rounded-[24px] border border-red-300/15 bg-[#172033] p-6"><p className="text-[11px] font-bold uppercase tracking-[.18em] text-red-300/55">{role === 'master' ? s('deleteWorld') : s('deleteCharacter')}</p><h2 className="mt-2 font-montserrat-alt text-[26px] font-extrabold text-white">{s('deleteTitle', {name: deleteTarget.name})}</h2><p className="mt-3 font-montserrat text-[13px] leading-relaxed text-white/50">{role === 'master' ? s('deleteDescriptionWorld') : s('deleteDescriptionCharacter')}</p><div className="mt-6 grid grid-cols-2 gap-2"><button type="button" onClick={() => setDeleteTarget(null)} disabled={isBusy} className="rounded-[13px] border border-white/15 py-3 font-bold text-white">{common('cancel')}</button><button type="button" onClick={() => void confirmDelete()} disabled={isBusy} className="rounded-[13px] bg-red-400 py-3 font-bold text-[#172033] disabled:opacity-40">{isBusy ? t('deleting') : common('delete')}</button></div></section></div></ViewportPortal> : null}
      {isLeaveConfirmationOpen ? <ViewportPortal><div className="fixed inset-0 z-[1000] grid place-items-center bg-[#070C17]/85 p-5 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="leave-character-editor-title"><section className="w-full max-w-[520px] rounded-[24px] border border-white/15 bg-[#172033] p-6 shadow-[0_28px_80px_rgba(0,0,0,.5)]"><p className="text-[11px] font-bold uppercase tracking-[.18em] text-white/40">{t('unsaved')}</p><h2 id="leave-character-editor-title" className="mt-2 font-montserrat-alt text-[27px] font-extrabold text-white">{t('saveBeforeLeaving')}</h2><p className="mt-3 font-montserrat text-[13px] leading-relaxed text-white/55">{t('leaveHelp')}</p>{!canSaveDraftOnExit ? <div className="mt-4 rounded-[12px] border border-white/10 bg-white/[.04] px-4 py-3"><p className="font-montserrat text-[11px] font-semibold text-white/55">{role === 'master' ? t('worldNameRequired') : t('nameDescriptionRequired')}</p></div> : null}<button type="button" onClick={() => void saveAndExitEditor()} disabled={!canSaveDraftOnExit || isSavingDraftOnExit} className="mt-5 h-[50px] w-full rounded-[13px] bg-white font-montserrat-alt text-[14px] font-extrabold text-[#172033] transition hover:bg-white/90 disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30">{isSavingDraftOnExit ? role === 'master' ? t('savingWorld') : t('savingCharacter') : t('saveExit')}</button><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setIsLeaveConfirmationOpen(false)} disabled={isSavingDraftOnExit} className="rounded-[13px] border border-white/15 py-3 font-bold text-white transition hover:bg-white/[.05] disabled:opacity-40">{t('keepEditing')}</button><button type="button" onClick={confirmEditorExit} disabled={isSavingDraftOnExit} className="rounded-[13px] border border-red-300/25 py-3 font-bold text-red-200 transition hover:bg-red-400/10 disabled:opacity-40">{t('exitWithoutSaving')}</button></div></section></div></ViewportPortal> : null}
    </ScaledPageViewport>
  );
}
