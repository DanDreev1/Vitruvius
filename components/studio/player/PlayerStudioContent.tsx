...
import { saveStudioCharacter } from '@/features/studio/player/api';
...
export default function PlayerStudioContent({ tab, entity, onEntityChange, registerDraftExitActions }: Props) {
  ...
  const update = (recipe: (current: StudioCharacterDraft) => StudioCharacterDraft) => { setDraft(recipe); setStatus(null); };
  ...
  }, [canSaveCore, entity.isDraft, registerDraftExitActions, save]);

  if (!entity.isDraft) return <div className="grid h-full place-items-center text-center"><div><h2 className="font-montserrat-alt text-[26px] font-extrabold text-white">Permanent character</h2><p className="mt-3 max-w-[520px] text-[13px] leading-relaxed text-white/50">Safe editing of existing characters will be enabled through the same atomic workflow. Start a new character to use the local-only creator.</p></div></div>;

  if (tab === 'character') return <StudioCharacterPage
    draft={toTabletDraft(draft)}
    status={status}
    onChange={(nextDraft) => update((current) => ({ ...current, ...nextDraft }))}
    onNameChange={(name) => { update((current) => ({ ...current, name })); onEntityChange({ name: name || 'New character' }); }}
