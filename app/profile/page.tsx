...
export default function ProfilePage() {
  ...
  const saveAvatar = async (file: File | null) => {
    ...
    if (!LOBBY_AVATAR_ALLOWED_MIME_TYPES.includes(file.type) || file.size > LOBBY_AVATAR_MAX_FILE_SIZE_BYTES) {
      setError(t('avatarValidation')); return;
    }
    setIsBusy(true); setError(null); setMessage(null);
    try {
  ...
  const logout = async () => {
    ...
    catch { setError(t('logoutError')); setIsBusy(false); }
  };
  ...
  const deleteAccount = async () => {
    ...
    try {
      ...
      router.replace('/');
    } catch { setError(t('deleteError')); setIsBusy(false); }
  };
      ...
      <main className="mx-auto flex h-[780px] w-full max-w-[1120px] items-center overflow-y-auto px-8 py-6">
        <section className="w-full overflow-hidden rounded-[30px] border border-white/10 bg-[#182135] shadow-2xl">
          <div className="border-b border-white/[.07] px-6 py-6 sm:px-9">
            <p className="font-montserrat text-[11px] font-bold uppercase tracking-[.22em] text-white/40">{t('account')}</p>
            <h1 className="mt-2 font-montserrat-alt text-[34px] font-extrabold text-white">{t('title')}</h1>
            <p className="mt-2 font-montserrat text-[14px] text-white/50">{t('subtitle')}</p>
          </div>
          {isLoading ? <p className="p-9 text-white/60">{t('loading')}</p> : user ? (
            <div className="grid grid-cols-[1fr_300px] gap-7 p-9">
              <div className="space-y-5">
                <div className="flex items-center gap-5 rounded-[22px] border border-white/[.08] bg-[#111A2D] p-5">
                  <button type="button" onClick={() => avatarInputRef.current?.click()} className="h-[92px] w-[92px] shrink-0 rounded-full border-2 border-white/15 bg-[#252F43] bg-cover bg-center transition hover:border-white/45" style={user.avatarUrl ? { backgroundImage: `url(${user.avatarUrl})` } : undefined} aria-label={t('avatarAria')} />
                  ...
                  <div><p className="font-montserrat-alt text-[20px] font-extrabold text-white">{t('avatarTitle')}</p><p className="mt-2 font-montserrat text-[12px] leading-relaxed text-white/45">{t('avatarDescription')}</p></div>
                </div>
                <div className="rounded-[22px] border border-white/[.08] bg-[#111A2D] p-5">
                  <label className="font-montserrat-alt text-[18px] font-extrabold text-white">{t('nickname')}</label>
                  <div className="mt-3 flex gap-2"><input value={nickname} maxLength={36} onChange={(event) => setNickname(event.target.value)} className="h-[50px] min-w-0 flex-1 rounded-[14px] border border-white/10 bg-[#0B1020] px-4 font-montserrat text-[14px] font-bold text-white outline-none focus:border-white/35" placeholder={t('nicknamePlaceholder')} /><button type="button" onClick={() => void saveNickname()} disabled={!nickname.trim() || nickname.trim() === user.nickname || isBusy} className="rounded-[14px] bg-white px-6 font-montserrat text-[13px] font-extrabold text-[#172033] disabled:opacity-35">{common('save')}</button></div>
                  <p className="mt-3 font-montserrat text-[12px] text-white/40">{t('nicknameHint')}</p>
                </div>
                <LanguageSwitcher />
              ...
              <aside className="flex flex-col gap-4">
                <div className="rounded-[20px] border border-white/[.08] p-5"><p className="text-[11px] uppercase tracking-widest text-white/35">{t('email')}</p><p className="mt-2 break-all font-montserrat text-[14px] font-bold text-white/75">{user.email}</p></div>
                <button type="button" onClick={() => void logout()} disabled={isBusy} className="rounded-[16px] border border-white/15 px-5 py-4 font-montserrat text-[14px] font-bold text-white transition hover:bg-white/[.05]">{t('logout')}</button>
                <button type="button" onClick={() => { setShowDelete(true); setDeleteSeconds(10); setDeleteConfirmation(''); setError(null); }} disabled={!user.nickname || isBusy} className="mt-auto rounded-[16px] border border-red-300/20 px-5 py-4 font-montserrat text-[13px] font-bold text-red-300/75 transition hover:bg-red-400/[.06]">{t('deleteAccount')}</button>
              </aside>
            </div>
          ) : null}
        </section>
      </main>
      {showDelete && user ? <div className="fixed inset-0 z-[1000] grid place-items-center bg-[#070C17]/90 p-4 backdrop-blur-md"><section className="w-full max-w-[500px] rounded-[26px] border border-red-300/15 bg-[#172033] p-6 shadow-2xl"><p className="text-[11px] font-bold uppercase tracking-[.2em] text-red-300/60">{t('permanentAction')}</p><h2 className="mt-2 font-montserrat-alt text-[28px] font-extrabold text-white">{t('deleteTitle')}</h2><p className="mt-3 text-[13px] leading-relaxed text-white/55">{t('deleteDescription')}</p>{deleteSeconds > 0 ? <div className="mt-5 rounded-[16px] bg-white/[.04] p-4 text-center font-montserrat text-[14px] text-white/60">{t('unlock', { seconds: deleteSeconds })}</div> : <div className="mt-5"><p className="mb-2 text-[12px] text-white/50">{t('typeToConfirm', { nickname: user.nickname })}</p><input value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} className="h-[48px] w-full rounded-[13px] border border-white/10 bg-[#0B1020] px-4 text-white outline-none focus:border-red-300/40" /></div>}<div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setShowDelete(false)} disabled={isBusy} className="rounded-[13px] border border-white/15 py-3 font-bold text-white">{common('cancel')}</button><button type="button" onClick={() => void deleteAccount()} disabled={deleteSeconds > 0 || deleteConfirmation !== user.nickname || isBusy} className="rounded-[13px] bg-red-400 py-3 font-bold text-[#172033] disabled:opacity-30">{t('deleteForever')}</button></div></section></div> : null}
    </ScaledPageViewport>
  );
}
